...
(select c.city_name, sum(ps.new_passengers) new_passengers, "Top 3" city_category
from dim_city c join fact_passenger_summary ps
on c.city_id=ps.city_id group by 1 order by 2 desc limit 3)
union 
(select c.city_name, sum(ps.new_passengers) new_passengers, "Bottom 3" city_category
from dim_city c join fact_passenger_summary ps
on c.city_id=ps.city_id group by 1 order by 2 asc limit 3);
...
concat(round(sum(repeat_passengers)*100/sum(total_passengers),2),"%") `repeation_rate_%`
from cte1 group by 1),

cte3 as (select month, city_name, sum(Total_passengers) total_passengers,
sum(repeat_passengers) repeat_passengers, 
concat(round(sum(repeat_passengers)*100/sum(total_passengers),2),"%") `monthly_repeation_rate_%`
from cte1 group by 1,2)

select cte3.*, cte2.`repeation_rate_%` `citywise_overall_repeatation_rate_%`
